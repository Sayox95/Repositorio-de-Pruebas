...
<div id="seccionRevisionFacturas" style="display:none; padding: 20px;">
  <button onclick="regresarMenu()">🔙 Regresar al Menú Principal</button>
  <h2 style="text-align:center;">Revisión de Facturas</h2>

  <div style="margin: 20px 0; display: flex; flex-wrap: wrap; gap: 10px;">
    <select id="filtroSector"><option value="">Filtrar por Sector</option></select>
    <select id="filtroEstado"><option value="">Filtrar por Estado</option></select>
    <input id="filtroFactura" type="text" placeholder="Buscar Nº Factura..." />
  </div>

  <div id="mensaje_carga_facturas" style="text-align:center; color:#007BFF; font-weight:bold;">Cargando información...</div>

  <div style="overflow-x: auto;">
    <table id="tablaFacturas" style="width: 100%; border-collapse: collapse; min-width: 1200px; display:none;">
      <thead>
        <tr>
          <th>#</th>
          <th><input type="checkbox" onclick="toggleTodosCheckboxes(this)"/></th>
          <th>Sector</th>
          <th>Placa</th>
          <th>Proceso</th>
          <th>Nombre</th>
...
<div id="menuPrincipal" style="display: flex; flex-direction: column; align-items: center; margin-top: 50px;">
    ...
    <button style="width: 250px; margin: 10px; padding: 10px;" onclick="mostrarSeccion('reportarFactura')">📥 Reportar Factura</button>
    <button style="width: 250px; margin: 10px; padding: 10px;" onclick="mostrarSeccion('revisionFacturas')">🔍 Revisión de Facturas</button>
    <button style="width: 250px; margin: 10px; padding: 10px;" onclick="mostrarSeccion('registrarAnticipo')">💵 Registrar Anticipo</button>
  </div>
...
<table class="header-table">
  <tr>
    <td rowspan="3" style="width:100px;">
      ...
      <img src="https://raw.githubusercontent.com/Sayox95/Gastos-de-Combustible/refs/heads/main/imagenes/Logo.png" alt="Logo UTCD">
    </td>
...
<div id="modal-signature" style="display:none; position:fixed; top:0; left:0; width:100%; height:100%; background:rgba(0,0,0,0.6); align-items:center; justify-content:center; z-index:3000; ">
  <div style="background:#fff; padding:20px; border-radius:8px; max-width:500px; width:90%;">
    ...
    <canvas id="canvas-signature" width="460" height="180" style="border:1px solid #ccc; border-radius:4px;"></canvas>
    <div style="margin-top:10px; text-align:right;">
      <div class="signature-buttons"><button type="button" id="clear-signature">Limpiar</button>
      <button type="button" id="confirm-signature">Confirmar</button></div>
    </div>
  </div>
</div>
...
<div id="seccionRevisionFacturas" style="display: none; padding: 1rem;">
  ...
  <div style="margin-bottom: 1rem;">
    ...
    <label for="buscarFactura" style="margin-left: 1rem;">Buscar factura:</label>
    <input type="text" id="buscarFactura" placeholder="Número de factura">
  </div>

  <div style="margin-bottom: 1rem;">
    <button onclick="actualizarEstadoSeleccionados('Revisada')">✔ Marcar como Revisada</button>
    <button onclick="actualizarEstadoSeleccionados('Anulada')">✖ Marcar como Anulada</button>
  </div>
...
<script>
const urlLeerFacturas = "/api/leer";
...
function aplicarFiltros() {
  ...
  const buscarFactura = document.getElementById("buscarFactura").value.toLowerCase();
...
<script>
document.addEventListener("DOMContentLoaded", () => {
  const btnRevision = document.getElementById("btnRevisionFacturas");
  if (btnRevision) {
    btnRevision.addEventListener("click", () => {
      document.getElementById("seccionRevisionFacturas").style.display = "block";
      document.getElementById("seccionMenuPrincipal").style.display = "none";
      cargarFacturasDesdeSheet();
    });
  }
});
...
function toggleTodosCheckboxes(master) {
  const checkboxes = document.querySelectorAll("#tablaFacturas tbody input[type='checkbox']");
  checkboxes.forEach(c => c.checked = master.checked);
}

async function cargarFacturas() {
  try {
    const response = await fetch('/api/leer'); // Reemplaza con tu endpoint real
    ...
    datos.forEach((factura, idx) => {
      ...
      fila.innerHTML = `
        ...
        <td>${factura.Placa}</td>
        <td>${factura.Proceso}</td>
        <td>${factura.Nombre}</td>
        <td>${factura.Identidad}</td>
        <td>${factura.TotalGastado} L</td>
        <td>${factura.Fecha}</td>
        <td>${factura.NumeroFactura}</td>
        <td>${factura.Estado}</td>
        <td><a href="${factura.URL_PDF}" target="_blank">Ver PDF</a></td>
      `;
      tbody.appendChild(fila);
      sectores.add(factura.Sector);
      estados.add(factura.Estado);
    });

    const sSel = document.getElementById('filtroSector');
    const eSel = document.getElementById('filtroEstado');
    sSel.innerHTML = '<option value="">Filtrar por Sector</option>';
    eSel.innerHTML = '<option value="">Filtrar por Estado</option>';
    sectores.forEach(s => sSel.innerHTML += `<option value="${s}">${s}</option>`);
    estados.forEach(e => eSel.innerHTML += `<option value="${e}">${e}</option>`);

    document.getElementById("mensaje_carga_facturas").style.display = "none";
    document.getElementById("tablaFacturas").style.display = "table";

    aplicarFiltros();
    sSel.onchange = aplicarFiltros;
    eSel.onchange = aplicarFiltros;
    document.getElementById('filtroFactura').oninput = aplicarFiltros;
  } catch (err) {
    console.error("Error al cargar facturas:", err);
    document.getElementById("mensaje_carga_facturas").innerText = "Error al cargar datos.";
  }
}
...
function aplicarFiltros() {
  ...
  filas.forEach(f => {
    ...
    const matchesSector = !sector || tds[2].textContent.toLowerCase().includes(sector);
    const matchesEstado = !estado || tds[10].textContent.toLowerCase().includes(estado);
    const matchesFactura = !factura || tds[9].textContent.toLowerCase().includes(factura);
    f.style.display = (matchesSector && matchesEstado && matchesFactura) ? '' : 'none';
  });
}
</script>
