 ...
 * - R2: sirve directo desde el bucket PDF_BUCKET
...
export async function onRequest({ request, env }) {
  ...
  // ── R2: servir directo desde el bucket ──────────────────────────────────
  if (target.hostname.includes("r2.dev") && env.PDF_BUCKET) {
    try {
      const filename = decodeURIComponent(target.pathname.split("/").pop());
      const obj = await env.PDF_BUCKET.get(filename);
      if (!obj) return new Response("PDF no encontrado", { status: 404, headers: corsHeaders() });
      ...
      return new Response("Error R2: " + e.message, { status: 500, headers: corsHeaders() });
    }
  }

  // ── Google Drive: lógica original con confirm token ──────────────────────
